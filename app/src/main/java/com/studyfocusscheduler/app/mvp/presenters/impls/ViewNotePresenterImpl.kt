...
class ViewNotePresenterImpl: ViewNotePresenter, AbstractBasePresenter<ViewNoteView>() {


    override fun onUiReady(lifeCycleOwner: LifecycleOwner,context:Context,noteVO: NoteVO) {
        //Toast.makeText(context, "Presenter Setup", Toast.LENGTH_SHORT).show()
        mView?.displayNoteInfo(noteVO)
    }
